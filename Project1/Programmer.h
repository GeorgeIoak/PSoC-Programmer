...
int StartProgram()
{
	...
	if (FAILED(hr))
	{
		std::cout << "Failed to get class id for PSoC Programmer COM object !" << std::endl;
		CoUninitialize();
		return 0;
	}
	...
	if (FAILED(hr))
	{
		std::cout << "Failed to create instance of PSoC Programmer COM object !" << std::endl;
		CoUninitialize();
		return 0;
	}
	hr = GetDispIDsByName();
	if (FAILED(hr))
	{
		std::cout << "Failed to get DispIDs of used methods";
		CoUninitialize();
		return 0;
	}
